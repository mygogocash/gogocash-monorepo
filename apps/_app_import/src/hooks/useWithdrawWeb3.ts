...
import { ResponseWithdrawCheck, ResWithdrawBankTransfer } from "@/interfaces/withdraw";
import React from "react";
import { BankSelected } from "@/features/wallet/component/MyWalletWithdraw";
import { useSession } from "next-auth/react";
import { useQuery } from "@tanstack/react-query";
import { checkThai, rateCurrency } from "@/lib/utils";
...
const useWithdrawWeb3 = () => {
  ...
  const {
    data: getCheckWallet,
    isLoading: isLoadingGetCheckWallet,
    refetch: refetchGetCheckWallet,
    error: getCheckWalletError,
  } = useQuery<ResponseWithdrawCheck>({
    queryKey: ["getCheckWallet", session?.user?._id],
  ...
  useEffect(() => {
    if (checkThai) {
      setWithdrawAmount(getCheckWallet?.netAmountTHB?.toFixed(2) || "");
    } else {
      setWithdrawAmount(getCheckWallet?.netAmount?.toFixed(2) || "");
    }
  }, [getCheckWallet]);
  ...
  async function withdrawCashback(msg_: {
    ...
    try {
      ...
      if (receipt && msg_?.info) {
        const rate = await rateCurrency("USD");
        createWithdraw({
          ...
          amount_total: Number(msg_.info?.totalPayoutUSD),
          amount_net: Number(withdrawAmount),
  ...
  const createTransactionWithdrawBank = async (data: CreateWithdrawDto) => {
    try {
      ...
      if (dt?.data?.status === "success") {
        ...
        window.location.href = "/wallet?active=withdraw";
      }
    } catch (err: unknown) {
      const msg = isAxiosError(err)
        ? typeof err.response?.data === "object" &&
          ...
          : undefined
        : undefined;
      toast.error(msg || "Failed to create bank transfer withdraw request.");
    }
  };
  return {
    withdrawCashback,
