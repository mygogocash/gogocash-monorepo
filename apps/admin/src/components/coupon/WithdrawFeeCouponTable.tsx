...
export default function WithdrawFeeCouponTable() {
  ...
  const canManage = permissions.can("coupon:manage");
  ...
  const { data, isLoading, isError, error } = useQuery<ListResponse>({
    ...
    queryFn: () =>
      client
        .get("/admin/withdraw-fee-coupons", { params: { limit: 50, page: 1 } })
        .then((res) => res.data),
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      return client.post("/admin/withdraw-fee-coupons", payload).then((r) => r.data);
    },
    onSuccess: async () => {
      setFormOpen(false);
      setForm(EMPTY_FORM);
      setFormError(null);
      await queryClient.invalidateQueries({ queryKey: ["withdraw-fee-coupons"] });
    },
    onError: (err) => {
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="text-sm">
              <span className="mb-1 block text-gray-600 dark:text-gray-400">Code</span>
              <input
            ...
            <label className="text-sm">
              <span className="mb-1 block text-gray-600 dark:text-gray-400">Discount mode</span>
              <select
                className="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-900"
            ...
            <label className="text-sm">
              <span className="mb-1 block text-gray-600 dark:text-gray-400">Start date</span>
              <input
                type="date"
                className="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-900"
                value={form.start_at}
            ...
            <label className="text-sm">
              <span className="mb-1 block text-gray-600 dark:text-gray-400">End date</span>
              <input
                type="date"
                className="w-full rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-700 dark:bg-gray-900"
                value={form.end_at}
            ...
            <label className="text-sm">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, usage_per_user: e.target.value }))
                }
              />
            </label>
          ...
          {formError ? <p className="mt-3 text-sm text-error-500">{formError}</p> : null}
