...
- **Auth: demo stub in `fixtures`, real Firebase phone OTP in `backend`.** In the default `fixtures` mode the login screen verifies a fixed demo OTP (`123456`) and writes a demo session (`useAuthGuardSession` flips `isAuthed` reactively via `notifyMobileSessionChange`). In `backend` mode `CustomerAuthScreen` is **wired to real phone auth**: `firebasePhoneAuth.ts` (web, DOM reCAPTCHA) and `firebasePhoneAuth.native.ts` (native, `@react-native-firebase/auth` — Metro `.native` split), then `firebaseLogin.ts` exchanges the ID token at `/auth/log-in` (project `gogocash-staging`). The remaining open task is **owner infra for native real OTP** — `apps/app/google-services.json` + SHA registration + an EAS `development-otp` build ([docs/api-integration.md](./docs/api-integration.md) §5).
- **`/profile` tab renders login INLINE when logged out** (not `<Redirect>`): the cross-navigator Tabs→`/login` redirect crashed the New-Arch Android view mounter on cold start, so `app/(tabs)/profile.tsx` renders `<CustomerAuthScreen mode="login" />` in-tab; after login `isAuthed` flips → profile hub.
