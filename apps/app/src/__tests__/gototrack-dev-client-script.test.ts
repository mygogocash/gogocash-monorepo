...
const devClient = await import("../../scripts/gototrack-dev-client.mjs");

describe("GoGoTrack Android dev-client script", () => {
  it("routes the npm command through the guarded launcher", () => {
    expect(packageJson.scripts?.["gototrack:dev-client"]).toBe(
      "node scripts/gototrack-dev-client.mjs"
    );
  });
