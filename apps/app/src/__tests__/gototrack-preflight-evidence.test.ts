...
describe("GoGoTrack preflight evidence bundle", () => {
  ...
  it("lists checkpoint UI hierarchy files for final Android acceptance evidence", async () => {
    tempDir = await mkdtemp(join(tmpdir(), "gototrack-evidence-summary-"));
    ...
    preflight.writeEvidenceBundle(
      {
        ...
        results: [
          ...
          { detail: "gogocash://gototrack", name: "GoGoTrack hub return", status: "pass" },
    ...
    expect(checklist).toContain("- gototrack-hub-ui.xml");
  ...
  it("fails the required activation nudge gate when hub UI evidence is missing the nudge", async () => {
    tempDir = await mkdtemp(join(tmpdir(), "gototrack-nudge-evidence-"));
...
if [ "$1" = "exec-out" ] && [ "$2" = "uiautomator" ]; then
  printf '<hierarchy><node text="GoGoTrack" content-desc="GoGoTrack hub" /></hierarchy>'
  exit 0
fi
...
`
    ...
    const fetchMock = vi.fn(async (url: string | URL, init?: RequestInit) => {
      ...
      if (requestUrl.endsWith("/gototrack/merchants")) {
        return new Response(
          JSON.stringify([{ enabled: true, merchant_id: "shopee-th", android_packages: ["com.shopee.th"] }]),
          { headers: { "content-type": "application/json" }, status: 200 }
        );
      }
      if (requestUrl.endsWith("/gototrack/settings")) {
        return new Response(JSON.stringify({ enabled: true }), {
          headers: { "content-type": "application/json" },
          status: 200,
        });
      }
      if (requestUrl.endsWith("/gototrack/detect")) {
        return new Response(
          JSON.stringify({
      ...
      if (requestUrl.endsWith("/gototrack/activate")) {
    ...
    const nudgeResult = report.results.find((item) => item.name === "GoGoTrack activation nudge visible");

    expect(resultSummary).toContain("GoGoTrack hub return:pass");
    expect(nudgeResult?.status).toBe("fail");
    await expect(readFile(join(tempDir, "gototrack-hub-ui.xml"), "utf8")).resolves.toContain("GoGoTrack hub");
  });

  it("taps the activation nudge center from the captured UI hierarchy", async () => {
    tempDir = await mkdtemp(join(tmpdir(), "gototrack-nudge-tap-"));
    const fakeAdb = join(tempDir, "adb");
    const foregroundFile = join(tempDir, "foreground.txt");
...
`
  ...
  it("fails the required activation nudge gate when the hub return step is omitted", async () => {
    tempDir = await mkdtemp(join(tmpdir(), "gototrack-nudge-precondition-"));
    const fakeAdb = join(tempDir, "adb");
...
`
    ...
    expect(nudgeResult).toMatchObject({
      status: "fail",
      detail: "--require-nudge needs --return-to-gototrack so gototrack-hub-ui.xml can be captured",
    });
    expect(tapResult).toMatchObject({
      status: "fail",
      detail: "--tap-nudge needs --return-to-gototrack so gototrack-hub-ui.xml can be captured",
    });
  });
});
