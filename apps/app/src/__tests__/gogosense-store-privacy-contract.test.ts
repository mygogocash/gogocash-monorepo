...
describe("GoGoSense store privacy contract", () => {
  it("matches the native detector permissions and platform support", () => {
    const appConfigSource = readAppFile("app.config.ts");
    const pluginSource = readAppFile("plugins/withGogosenseUsageAccess.js");
    const moduleConfig = JSON.parse(readAppFile("modules/gogosense-detector/expo-module.config.json")) as {
      platforms?: string[];
    };
    const privacyContract = readAppFile("docs/gogosense-store-privacy-contract.md");
    ...
    expect(privacyContract).toContain("No GoGoSense native detector");
