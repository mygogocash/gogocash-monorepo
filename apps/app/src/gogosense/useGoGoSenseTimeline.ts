...
export type GoGoSenseTimelineEntry = {
  id: string;
  title: string;
  body: string;
  status: string;
...
function mapTimeline(data: unknown): GoGoSenseTimelineEntry[] {
...
/**
 * Loads the GoGoSense detection timeline from the authed api. Returns `null`
 ...
 */
export function useGoGoSenseTimeline(
  apiOverride?: TimelineApi | null,
): GoGoSenseTimelineEntry[] | null {
  const liveApi = useGoGoSenseApi();
  const api = apiOverride ?? liveApi;
  const [entries, setEntries] = useState<GoGoSenseTimelineEntry[] | null>(null);
