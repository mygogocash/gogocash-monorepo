...
import { findDefaultAdb, parseDevices } from "./gogosense-preflight.mjs";
...
export function configureAdbReverse({
  ...
  if (devicesResult.error || devicesResult.status !== 0) {
    logger.warn?.(
      `[gogosense:dev-client] adb unavailable; skipping reverse tcp:${port}. ${devicesResult.error?.message ?? devicesResult.stderr ?? ""}`.trim()
    );
  ...
  if (devices.length === 0) {
    logger.warn?.(`[gogosense:dev-client] no Android device connected; skipping reverse tcp:${port}`);
    return { adb, port, reversed: [], skipped: true };
  }
  ...
  for (const device of devices) {
    ...
    if (reverseResult.error || reverseResult.status !== 0) {
      logger.warn?.(
        `[gogosense:dev-client] failed to reverse tcp:${port} on ${device.serial}: ${reverseResult.error?.message ?? reverseResult.stderr ?? ""}`.trim()
      );
      continue;
    }
    reversed.push(device.serial);
    logger.log?.(`[gogosense:dev-client] adb reverse tcp:${port} configured for ${device.serial}`);
  }
