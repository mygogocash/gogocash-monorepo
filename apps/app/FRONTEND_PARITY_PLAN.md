...
>
> **Update (2026-06-24):** Desktop home polish — (1) **`CustomerDesktopFooter` scrolls with page content** again (footer inside home `ScrollView` + `desktopFooterCap`, not pinned below a `flex:1` scroll sibling); full-bleed width matches header via `getDesktopShellOffset(width)` breakout. (2) Desktop Top/Trending/Travel/Makeup rails show **2 rows** (`getDesktopBrandColumnsPerRow`, `brandSectionFrameWidth`). (3) Brand card cashback copy unified to **`Cashback upto`**. (4) RN Web console hygiene — deprecated style props (`pointerEvents` prop, legacy shadow keys) guarded by parity tests. Gates: `npm --prefix apps/app run test` / `test:render` / `typecheck`.
...
- **2026-07:** Native cookie banner dismiss persists via `cookieConsentStorage.ts` (SecureStore / `localStorage`) — no longer re-shows every cold start after accept.
