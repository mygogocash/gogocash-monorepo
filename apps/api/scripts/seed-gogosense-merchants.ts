...
import { defaultGogosenseMerchants } from '../src/gogosense/seeds/default-gogosense-merchants';

type GogosenseMerchantSeedDocument = {
  merchant_id: string;
  brand_slug: string;
...
function selectorMatchesMerchant(
  ...
  merchant: (typeof defaultGogosenseMerchants)[number],
): boolean {
  if (selectors.size === 0) {
    return false;
  }
...
export function buildSeedOperations(options: SeedOptions): SeedOperation[] {
  return defaultGogosenseMerchants.map((merchant, index) => {
...
function getMerchantModel(): Model<GogosenseMerchantSeedDocument> {
  const modelName = 'GogosenseMerchant';
  const merchantSchema = new Schema<GogosenseMerchantSeedDocument>(
    {
  ...
  return (mongoose.models[modelName] ||
    ...
    GogosenseMerchantSeedDocument
  >;
}

async function seedGogosenseMerchants(options: SeedOptions) {
  ...
  if (options.dryRun) {
    console.log(
      `[seed-gogosense-merchants] dry-run defaultMerchants=${operations.length} selectedForEnable=${selectedCount} resetEnabled=${options.resetEnabled}`,
    );
  ...
  try {
    for (const merchant of defaultGogosenseMerchants) {
      const enableSelected =
        (options.enableFirst &&
          merchant.merchant_id === defaultGogosenseMerchants[0].merchant_id) ||
        selectorMatchesMerchant(options.enableSelectors, merchant);
      const setEnabled =
        enableSelected || options.resetEnabled ? enableSelected : undefined;
    ...
    console.log(
      `[seed-gogosense-merchants] upserted=${upserted} matched=${matched} selectedForEnable=${enabled} resetEnabled=${options.resetEnabled}`,
    );
  } finally {
...
if (require.main === module) {
  seedGogosenseMerchants(parseSeedOptions(process.argv.slice(2))).catch(
    (error) => {
      console.error('[seed-gogosense-merchants]', error.message);
      process.exit(1);
    },
  );
}
