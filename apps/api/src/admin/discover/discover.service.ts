...
export class DiscoverService {
  ...
  async addItem(
    type: string,
    item: { offer_id: string; custom_title?: string },
  ) {
  ...
  async updateItem(
    type: string,
    itemId: string,
    data: { custom_title?: string; sort_order?: number },
  ) {
    const update: Record<string, any> = {};
    if (data.custom_title !== undefined)
      update['items.$.custom_title'] = data.custom_title;
    if (data.sort_order !== undefined)
      update['items.$.sort_order'] = data.sort_order;
