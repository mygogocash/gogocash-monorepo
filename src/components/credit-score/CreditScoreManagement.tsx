...
import type { CreditScore, CreditTier, ScoringConfig } from "@/types/adminModules";
...
export default function CreditScoreManagement() {
  ...
  const [filters, setFilters] = useState({ search: "", tier: "", minScore: "", maxScore: "" });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {(["bronze", "silver", "gold", "platinum"] as CreditTier[]).map((t) => (
      ...
      {listQ.isError ? (
        ...
        <p className="py-12 text-center text-sm text-gray-500 dark:text-gray-400">No rows for this filter.</p>
      ) : (
        <>
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {rows.map((r: CreditScore) => (
                  <tr key={r.userId} className="text-gray-800 dark:text-gray-200">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500">{r.lastUpdated.slice(0, 10)}</td>
                    <td className="px-4 py-3">
                      <Button size="sm" variant="outline" onClick={() => setUserId(r.userId)}>
                        Details
                      </Button>
                    </td>
                  </tr>
      ...
      <Modal isOpen={Boolean(userId)} onClose={() => setUserId(null)} className="max-h-[90vh] max-w-2xl overflow-y-auto p-6">
        {detailQ.isLoading ? (
          ...
          <div className="space-y-6">
            <div>
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white">
                {detailQ.data.userName} — {detailQ.data.currentScore} ({detailQ.data.tier})
              </h4>
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1 text-sm text-gray-600 dark:text-gray-400">
                {detailQ.data.factors.map((f) => (
                  <li key={f.name}>
                    {f.name}: weight {f.weight} · contribution {Math.round(f.contribution)}
                  </li>
                ))}
              </ul>
            </div>
            <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
              <p className="font-medium text-gray-900 dark:text-white">Manual override</p>
              ...
              <Button className="mt-3" size="sm" onClick={() => void overrideM.mutateAsync()} disabled={!overrideScore}>
                Apply override
              </Button>
            </div>
            <div>
              ...
              <div className="mt-2 max-h-40 overflow-y-auto text-xs">
                {(auditQ.data ?? []).map((a, i) => (
                  <div key={i} className="border-b border-gray-100 py-1 dark:border-gray-800">
                    {a.timestamp}: {a.fromScore} → {a.toScore} ({a.reason}) by {a.adminId}
                  </div>
                ))}
              </div>
            </div>
      ...
      <Modal isOpen={configOpen} onClose={() => setConfigOpen(false)} className="max-w-lg p-6">
        {!cfgDraft ? (
          <p className="text-sm text-gray-500">Loading…</p>
        ) : (
          <div className="space-y-3">
            <h4 className="font-semibold text-gray-900 dark:text-white">Scoring weights (%)</h4>
            <label className="text-sm text-gray-600 dark:text-gray-400">Transactions</label>
            <Input
              type="number"
              value={String(cfgDraft.transactionWeight)}
              onChange={(e) => setCfgDraft({ ...cfgDraft, transactionWeight: Number(e.target.value) })}
            />
            <label className="text-sm text-gray-600 dark:text-gray-400">Referrals</label>
            <Input
              type="number"
              value={String(cfgDraft.referralWeight)}
              onChange={(e) => setCfgDraft({ ...cfgDraft, referralWeight: Number(e.target.value) })}
            />
            <label className="text-sm text-gray-600 dark:text-gray-400">Membership</label>
            <Input
              type="number"
              value={String(cfgDraft.membershipWeight)}
              onChange={(e) => setCfgDraft({ ...cfgDraft, membershipWeight: Number(e.target.value) })}
            />
            <p className="text-sm font-medium text-gray-900 dark:text-white">Tier thresholds</p>
            {cfgDraft.tiers.map((t, idx) => (
              <div key={t.name} className="grid grid-cols-3 gap-2 text-xs">
                <span className="py-2 capitalize text-gray-600 dark:text-gray-400">{t.name}</span>
                <Input
                  type="number"
                  value={String(t.min)}
                  onChange={(e) => {
            ...
            <Button onClick={() => cfgDraft && void saveCfg.mutateAsync(cfgDraft)}>Save config</Button>
          </div>
        )}
      </Modal>
    </div>
