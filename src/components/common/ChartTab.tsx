...
const ChartTab: React.FC<ChartTabProps> = ({
  ...
  return (
    <div className="flex max-w-full flex-wrap items-center justify-end gap-0.5 rounded-lg bg-gray-100 p-0.5 dark:bg-gray-900">
      {TABS.map((tab) => (
        <button
          key={tab.id}
          type="button"
          onClick={() => setSelected(tab.id)}
          className={`rounded-md px-2 py-2 text-theme-sm font-medium whitespace-nowrap transition-all duration-200 ease-out active:scale-[0.97] sm:px-3 ${
            selected === tab.id
