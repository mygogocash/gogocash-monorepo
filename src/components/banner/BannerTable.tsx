...
import AppOpenPopupModal from "./AppOpenPopupModal";
import { pathImage } from "@/utils/helper";
import { BannerData, BannerRequestForm } from "@/types/banner";

export default function BannerTable() {
  const [openModal, setOpenModal] = useState<boolean>(false);
  const [openAppOpenPopupModal, setOpenAppOpenPopupModal] = useState(false);
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <AppOpenPopupModal
        isOpen={openAppOpenPopupModal}
        onClose={() => setOpenAppOpenPopupModal(false)}
      />
      {/* Header */}
      <FormUpdate
        fetchData={refetch}
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-5">
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => setOpenAppOpenPopupModal(true)}
