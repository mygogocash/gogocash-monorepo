...
const severityStyles = {
  critical: "border-l-red-500 bg-red-50/80 dark:border-l-red-400 dark:bg-red-950/30",
...
export function ExecutiveAlertList({ alerts, title = "Risks & anomalies" }: Props) {
  return (
    <div className="rounded-xl border border-gray-200/90 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
      ...
      <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Prioritized for leadership review</p>
      <ul className="mt-3 space-y-2">
        {alerts.map((a) => (
          <li
            key={a.id}
            className={`rounded-r-lg border-l-4 py-2.5 pl-3 pr-2 ${severityStyles[a.severity]}`}
          >
            ...
            <p className="mt-1 text-xs leading-relaxed text-gray-600 dark:text-gray-300">{a.detail}</p>
