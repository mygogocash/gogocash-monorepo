...
export default function AdminUsersTable() {
  const { loading, error, getAdminUsers, deleteAdminUser, inviteAdminUser, clearError } =
    useApi();
  ...
  const [userToReset, setUserToReset] = useState<{ username: string; email: string } | null>(null);
      ...
      >
        <div className="text-center sm:text-left">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Send a password reset email to this admin user. They will receive a link to set a new password.
          </p>
          {userToReset && (
                  ...
                  {users &&
                    users?.map((user) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {user.status === "pending" ? (
                            <span className="inline-flex rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800 dark:bg-amber-900/50 dark:text-amber-200">
                              Pending
                            </span>
                          ) : (
