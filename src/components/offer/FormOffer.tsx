...
const FormOffer = ({
  ...
  // commits it into upsize_product_types (the table) and clears it for the next
  // line; "Cancel" just clears it.
  ...
  const cancelUpsizeDraft = () => setUpsizeDraft(EMPTY_UPSIZE_DRAFT);

  const addUpsizeDraft = () => {
    if (!upsizeDraft.name.trim()) return;
    setForm((prev) => ({
      ...prev,
      upsize_product_types: [...(prev.upsize_product_types ?? []), upsizeDraft],
    }));
    setUpsizeDraft(EMPTY_UPSIZE_DRAFT);
  };

  const removeUpsizeRow = (i: number) =>
    setForm((prev) => ({
      ...prev,
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
                                <table className="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                                  ...
                                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {(form.upsize_product_types ?? []).map(
                                      (line, i) => {
                                        const isCash = line.pay_in === "cash";
                                        const value = line.is_tagline
                                          ? "—"
                                          : isCash
                                            ? line.amount != null
                                              ? `${line.amount} ${line.currency || ""}`.trim()
                                              : "—"
                                            : line.commission_info
                                              ? `${line.commission_info}%`
                                              : "—";
                                        return (
                                          <tr
