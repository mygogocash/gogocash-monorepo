...
function partnerBestRateToStoreCommission(partnerPercent: number): number {
  return (
    Math.round(partnerPercent * STORE_COMMISSION_FROM_PARTNER_FACTOR * 100) /
    100
  );
}
...
const FormOffer = ({
  ...
  const logoMobileUrl = useObjectUrl(form.logo_mobile);
        ...
        header={
          <div className="flex w-full flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
            <div className="min-w-0 space-y-3">
              ...
              <p className="max-w-2xl text-sm leading-snug text-pretty text-gray-600 dark:text-gray-300">
            ...
            <div className="xsm:flex-row xsm:justify-end flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:items-center sm:gap-2">
              <Button
                className="xsm:order-1 xsm:w-auto order-2 min-h-11 w-full touch-manipulation"
                size="sm"
                variant="outline"
                type="button"
              ...
              <Button
                className="xsm:order-2 xsm:w-auto order-1 min-h-11 w-full touch-manipulation"
                size="sm"
                type="button"
                disabled={isLoading}
                onClick={handleSave}
                startIcon={
                  isLoading ? (
                    <div className="border-t-brand-500 h-3 w-3 animate-spin rounded-full border-2 border-gray-300 dark:border-gray-600" />
                  ) : null
                }
              >
                Save changes
        ...
        <>
          ...
          >
              ...
              <p className="text-xs leading-relaxed text-gray-500 dark:text-gray-400">
                {form.all_product_types ? (
                  ...
                  <>
                    ...
                    info, then use{" "}
                    <span className="font-medium">Save changes</span> here or in
                    the header to persist.
                  </>
                )}
              </p>
              {!form.all_product_types &&
                        ...
                        >
                          ...
                          <div className="flex shrink-0 sm:items-end sm:pb-0.5">
                            <Button
                              ...
                              type="button"
                              onClick={() => {
                                const next = (form.product_types ?? []).filter(
                                  (_, j) => j !== i,
                                );
                                setForm({
                                  ...form,
                                  product_types: next.length ? next : undefined,
                                });
                              }}
                              disabled={isLoading}
            ...
            >
              ...
              <p className="text-brand-800/80 dark:text-brand-200/80 mt-1 text-xs">
                Structured terms as supplied by the partner or affiliate
                network. This does not change when you edit “Commission (%)” or
                “Max cap” above — partner max cap is separate and read-only
                here.
              </p>
              ...
              offer.special_commissions.length > 0 ? (
                <p className="mt-3 text-xs text-gray-600 dark:text-gray-400">
                  ...
                  {offer.special_commissions.length} tier(s) — see partner
                  portal for full rules.
                </p>
              ) : null}
                ...
                <div className="mt-4 space-y-5">
                  <div>
                    ...
                    <p className="mt-0.5 ml-6 text-xs text-gray-600 dark:text-gray-400">
                      ...
                      . Pick a system category below, or leave “Use partner
                      feed” so the tag uses that value.
                    </p>
                    {form.offer_display_tags.brand_category_enabled ? (
          ...
          >
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, policy_category_id: e.target.value })
              }
            >
              <option value="">
            ...
            {form.policy_category_id ? (
              <p className="text-xs text-gray-600 dark:text-gray-400">
                Saving will pin this offer to the selected category’s policy
                text. Users should see that category’s terms when engaging with
                this offer (per your app implementation).
              </p>
            ) : (
              <p className="text-xs text-gray-600 dark:text-gray-400">
                No override: the app can match{" "}
                <span className="font-medium">{offer?.categories ?? "—"}</span>{" "}
          ...
          >
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Upload images for desktop, mobile, and banners. Leave empty to
              keep current.
            </p>
            ...
            <div>
              ...
              {(form.logo_mobile || (openModal as Offer).logo_mobile) && (
                <RemoteOrBlobImage
                  src={
                    logoMobileUrl ?? pathImage((openModal as Offer).logo_mobile)
                  }
                  alt="Preview"
                  width={256}
                  height={256}
