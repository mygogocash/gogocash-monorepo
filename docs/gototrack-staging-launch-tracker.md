...
| T-013 | Admin-staging environment check | **partial — env misconfig** | [`evidence/staging/T-013-admin-check.txt`](../evidence/staging/T-013-admin-check.txt) — HTTP 307→signin OK. Railway staging `gogocash-admin` vars wrong: `NEXT_PUBLIC_API_URL`/`NEXTAUTH_URL` point at **production** `*.up.railway.app`; **`NEXTAUTH_SECRET` missing**. **Owner:** fix vars + redeploy admin; login + DevTools confirm `api-staging.gogocash.co`. | 2026-07-02 |
